...
#' }
plot_umap_grid <- function(tbl_x, feature_list, quantile_limits_list = NULL, n_cols = 3, out_width = 180, output_path, show_labels = FALSE, ...) {
  ...
  plot_l <- pmap(list(feature_list, quantile_limits_list, show_labels_list), function(feature_x, quantile_limits_x, show_labels_x) {
    plot_umap(tbl_x = tbl_x, feature_x = !!sym(feature_x), plot_width = 0.6 * col_width, plot_height = 0.6 * col_width, output = "plot", title = feature_x, quantile_limits = quantile_limits_x, show_labels = show_labels_x, ...)
  })
