  ...
  if(mode == "discrete") {
    ...
    if(pretty_labels) {
      ...
      label_df <- tbl_x %>%
        ...
        # Label the clusters
        p <- p + ggrepel::geom_text_repel(data = label_df, aes(label = {{feature_x}}), size = label_size, color = "black", min.segment.length = 0, segment.size = 0.3, max.overlaps = nrow(tbl_x)*0.1)
      } else {
